class Sensor{
    constructor(car){
        this.car=car;
        this.rayCount=9; // se mettessimo 1 produrrebbe un errore a rigo 63, perchè avremmo i/0
        this.rayLength=150;
        this.raySpread=Math.PI+2; // PI/2 = 90°, PI/4 = 45°
    ...
    update(roadBorders, traffic){ // ERRORE di visualizzazione:  in assenza del parametro roadBorders, la pagina index non carica nulla
        this.#castRays(); 
        this.readings=[]; // inizializziamo l'array
        for(let i = 0; i < this.rays.length; i++) {
            this.readings.push(
                this.#getReading(this.rays[i], roadBorders, traffic) // metodo che prende i raggi e i bordi come parametri
            );
        }
    }
    ...
    #castRays(){
        ...
        for(let i = 0; i < this.rayCount; i++){
            ...
            const rayAngle = lerp(
                this.raySpread/2, // A: 45°
                -this.raySpread/2, // B: 45°
