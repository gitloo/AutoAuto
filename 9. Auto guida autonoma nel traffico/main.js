...
if(localStorage.getItem("bestBrain")){ // se in localStorage è stato immagazzinato un bestBrain (cioè la rete neurale dell'auto che è riuscita ad andare più avanti)...
    for(let i = 0; i < cars.length; i++){ //(per ogni macchina, il suo brain sarà uguale a quello contenuto nel localStorage
        cars[i].brain = JSON.parse( // ...andremo ad assegnare il brain della bestCar al valore
